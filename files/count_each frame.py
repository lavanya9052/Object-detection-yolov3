...
def final_detection(main_boxes,total_boxes,each_box_acc,each_box_class_index,height_ratio,width_ratio):
    ...
    for k in main_boxes:
        ...
        cv2.putText(frame,text,(x,y-3),font,0.7,(0,0,255),2)
        cv2.rectangle(frame,(x,y),(w+x,h+y),(0,255,0),2)


    length=20
    for class_name,count in count.items():
        text_image= str(class_name) + ':' + str(count)
        font = cv2.FONT_HERSHEY_PLAIN
        cv2.putText(frame,text_image,(10,length),font,1,(0,255,0),2)
        length+=20
